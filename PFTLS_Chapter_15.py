...
def simulation_step(CA, next_CA):
    ...
    for x in xrange(width):
        for y in xrange(height):
            ...
            for xpos in xrange(- inhibitor_radius, inhibitor_radius + 1):
                for ypos in xrange(- inhibitor_radius, inhibitor_radius + 1):
                    inhibiting_cells += CA[(y+ypos)%height, (x+xpos)%width]

            # count the number of activating cells within the radius
            for xpos in xrange(- activator_radius, activator_radius + 1):
                for ypos in xrange(- activator_radius, activator_radius + 1):
